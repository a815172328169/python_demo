"""
csv文件数据转换成txt，导入influxdb
"""
import csv

with open('./data/all_bu2.txt', 'w') as f1:  # 创建一个需要写入的txt文件
    f1.write('# DDL\n')
    ...
    for i in range(1, 2):
        s = str(i)
        with open('./data/all_bu2.csv') as csvfile:
            ...
            for row in list(readcsv)[1:]:
                ...
                f1.write(',error_code=' + row[6])  # tag写入
                f1.write(',media_type=' + row[7])  # tag写入
                f1.write(',message=' + row[8].replace(' ', ''))  # tag写入
                f1.write(',method=' + row[9])  # tag写入
                # f1.write(',method=' + 'GET')  # tag写入
                f1.write(',node_id=' + row[10])  # tag写入
                f1.write(',response_code=' + row[11])  # tag写入
                f1.write(',status=%d' % int(row[12]))  # tag写入
                f1.write(',task_id=' + row[15])  # tag写入
                f1.write(',task_type=' + row[16])  # tag写入
                # f1.write(',url=' + 'http://172.22.16.228:30080/verification4aiip/product/mcloudimageclassify3n2c/cmic/mcloud/v1.5/image/label/')  # tag写入
                f1.write(',url=' + row[17])
                f1.write(' available=%d' % int(row[2]))  # fields写入
